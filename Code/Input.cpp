...
int Input::GetInteger(Output* pO) const
{
	...
	int label = stoi(GetString(pO));


	// Note: stoi(s) converts string s into its equivalent integer (for example, "55" is converted to 55)

	return label; // this line should be changed with your implementation
}
...
ActionType Input::GetUserAction() const
{
	...
	if (UI.InterfaceMode == MODE_DESIGN)
	{
		...
		if (y >= 0 && y < UI.ToolBarHeight)
		{
			...
			case ITM_PASTE_CARD: return PST_CARD;
			case ITM_DELETE: return DEL_GM_OBJ;
			case ITM_SAVE: return SAVE_GRID;
			case ITM_OPEN: return OPEN_GRID;
	...
	else
	{
		...
		if (y >= 0 && y < UI.ToolBarHeight) {
			...
			case ITM_INPUT_DICE_VALUE: return INPT_DICE_VAL;
			case ITM_NEW_GAME: return NEW_GAME;
			case ITM_SWITCH_TO_DESIGN_MODE: return TO_DESIGN_MODE;
			case ITM_EXIT2: return EXITT;
