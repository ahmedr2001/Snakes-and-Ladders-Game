...
{
	///TODO: implement the following function as described in Output.h file
	return UI.ToolBarHeight + UI.CellHeight * (cellPos.VCell());  // this line should be changed with your implementation
}
...
void Output::DrawCardNumber(const CellPosition& cellPos, int cardNum) const
{
	...
	pWind->GetIntegerSize(w, h, UI.CardNumFont);
...
void Output::CreateDesignModeToolBar() const
{
	...
	MenuItemImages[ITM_OPEN] = "images\\Menu_Open.jpg";
...
void Output::DrawCell(const CellPosition& cellPos, int cardNum) const
{
	...
	if (cardNum != -1 && cellPos.GetCellNum() != 1 && cellPos.GetCellNum() != 99) // no card
	...
	if (cardNum != -1 && cellPos.GetCellNum() != 1 && cellPos.GetCellNum() != 99) // Note: cardNum -1 means no card
...
void Output::DrawLadder(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	if ((fromCell.HCell() != toCell.HCell()) || (toCell.VCell() >= fromCell.VCell()) || (fromCell.GetCellNum() == 1))
		return;
	...
	//int hLinesCnt = (y1 - y2) / UI.CellHeight;
	int hLinesCnt = fromCell.VCell() - toCell.VCell() - 1;
	for (int i = 0; i <= hLinesCnt; i++) {
		pWind->SetPen(UI.LadderColor, UI.LadderlineWidth);
		int y12 = fromStartY - i * UI.CellHeight;
